...
export class IndicatorsComponent implements OnInit {
  ...
  loadGridData(pageNo) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (this.selectedInstanceId) {
        this.indicatorService.getFacilityIndicator(this.page.size, pageNo, this.selectedInstanceId).then(data => {
          this.columns = []
          this.rows = []
  ...
  loadInstance() {
    return new Promise((resolve, reject) => {
      var data = this.instanceService
        .getInstanceStatusWise(EntityType.Facility, 2, this.allRecords, 1)
  ...
  loadDynamicolumnGridData(pageNo, searchText) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.dynamicColumnService.getColumnsForIndicator(this.paginationPageSize, pageNo, EntityType.Facility, this.selectedInstanceId, searchText).then(data => {
        this.dcgcolumns = []
        this.dcgrows = []
    ...
    if (this.dcgtotalSelected.length > 0) {
      ...
      this.dcgtotalSelected.map(a => {
        ...
        let data = {
          ...
          "entityTypeId": EntityType.Facility,
          "instanceId": this.selectedInstanceId
        };
        lstForIndicator.push(data);
      });

      this.indicatorService.saveFacility(lstForIndicator).then(a => {
        this.loadDynamicolumnGridData(1, this.dynamicColumnSearchText).then(a => {
          this.dcgtotalSelected = [];
          this.dcgSelected = [];
        });
  ...
  async removeIndicators() {
    ...
    if (await this.modalService.confirm()) {
      ...
      this.indicatorService.removeFacility(lstForIndicator)
        .then(response => {
          this.toast.success(MessageConstant.DeleteSuccess);
          this.loadDynamicolumnGridData(1, this.dynamicColumnSearchText).then(a => {
            this.dcgtotalSelected = [];
