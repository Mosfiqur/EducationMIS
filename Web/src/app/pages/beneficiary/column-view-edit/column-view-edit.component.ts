...
export class ColumnViewEditComponent implements OnInit {
  ...
  loadColumns(searchText) {    
    var data =this.dynamicColumnService.getAllColumns(this.allRecords,1,EntityType.Beneficiary,searchText) //this.httpClientService.getAsync(ApiConstant.GetAllColumn + '?EntityTypeId=' + EntityType.Beneficiary)
      .then((data) => {        
  ...
  loadGridView(id) {
    var data = this.httpClientService.getAsync(ApiConstant.GetByIdBeneficiary + '?id=' + id)
      .then((data) => {
  ...
  saveDynamicView() {
    ...
    this.gridView.entityTypeId = EntityType.Beneficiary;
   // this.gridView.name = this.name;
    ...
    this.httpClientService.postAsync(ApiConstant.UpdateBeneficiaryGridView, this.gridView)
      .then(a => {
